...
public class PricingServiceImpl implements PricingService {
	...
	public HttpServletRequest initialize(HttpServletRequest request) {
		...
		if (session != null) {
			...
			if (curPayment != null) {
				int daysLeft = TimeStampManager.daysBetween(
						TimeStampManager.getCurrentTime(),
						curPayment.getDateEnd());
				double savedCash = savedCash();
				request.setAttribute("savedCash",
						Math.round(savedCash * 100.0) / 100.0);
	...
	private void continueCurrentTariff(int months, Tariff tariff) {
		List<Payment> payments = new PaymentServiceImpl()
				.getAvailableUserPays(user.getId());
		Timestamp timeStart = payments.get(payments.size() - 1).getDateEnd();
		Timestamp timeEnd = TimeStampManager.addNumberOfMonth(
				(Timestamp) timeStart.clone(), months);
		double price = tariff.getPrice() * months;

		Payment payment = createPayment(user.getId(), tariff.getId(),
				timeStart, timeEnd, price, true, true);
	...
	private void newHigherTariff(int months, Tariff curTariff, Tariff newTariff) {
		...
		Object[] argCreatePayment = { user.getId(), newTariff.getId(),
				timeStart, timeEnd, price, 1, 1 };
	...
	private void createFirstuserPayment(Tariff tariff, int months) {
		...
		Object[] argCreatePayment = { user.getId(), tariff.getId(), timeStart,
				timeEnd, price, 1, 1 };
	...
	private double savedCash() {
		...
		for (int i = 0; i < payments.size(); i++) {
			int daysBetween = TimeStampManager.daysBetween(payments.get(i)
					.getDateCreated(), payments.get(i).getDateEnd());
			if (daysBetween != 0) {
				int daysLeft = TimeStampManager.daysBetween(TimeStampManager
						.getCurrentTime(), payments.get(i).getDateEnd());
				savedCash += payments.get(i).getPrice() / daysBetween
						* daysLeft;
			} else {
	...
	private Payment createPayment(long userId, long tariffId,
			Timestamp timeStart, Timestamp timeEnd, double price,
			boolean status, boolean available) {
